    ...
    public:
        ...
        void copyFromForce(const RESULT::Drvt &drvt) {
            ...
            if (PARAM::FLAG_B95 == true) {
                this->Bal = std::abs(drvt.div_v) / (std::abs(drvt.div_v) + sqrt(drvt.rot_v * drvt.rot_v) +
...
#else
            fprintf(fp, "%lld\t%lld\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", id, tag, mass,
                    pos.x, pos.y, pos.z, vel.x, vel.y, vel.z, dens, eng, pres, pot, ent, temp, snds, dt);
#endif
        }
...
#else
            fscanf(fp, "%lld\t%lld\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", &id, &tag,
                   &mass, &pos.x, &pos.y, &pos.z, &vel.x, &vel.y, &vel.z, &dens, &eng, &pres, &pot, &ent, &temp, &snds, &dt);
#endif
        }
