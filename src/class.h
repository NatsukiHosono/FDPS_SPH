    ...
    public:
        ...
        void writeAscii(FILE *fp) const {
            fprintf(fp, "%ld\t%ld\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", id, tag, mass, pos.x,
                    pos.y, pos.z, vel.x, vel.y, vel.z, dens, eng, pow(pres_smth, 1.0 / PARAM::DISPH_POWER), pot);
        }

        void readAscii(FILE *fp) {
            double phase, temp;
            fscanf(fp, "%ld\t%ld\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", &id, &tag, &mass, &pos.x,
                   &pos.y, &pos.z, &vel.x, &vel.y, &vel.z, &dens, &eng, &pres_smth, &pot);
        ...
        void initialize() {
            smth = PARAM::SMTH * pow(mass / dens, 1.0 / (PS::F64) (PARAM::Dim));
            pV = pow(EoS->Pressure(dens, eng, 120), PARAM::DISPH_POWER) * mass / dens;
            grad_smth = 1.0;
        }
        ...
        void fullDrift(const PS::F64 dt_glb) {
            ...
            smth = PARAM::SMTH * pow(mass / dens, 1.0 / (PS::F64) (PARAM::Dim));
            //smth = std::max(smth, PARAM::SMTH * pow(mass / dens, 1.0/(PS::F64)(PARAM::Dim)));
            if (PARAM::FLAG_R00 == true) {
                AVa += ((2.0 - AVa) * std::max(-div_v, 0.0) - (AVa - 0.1) / (smth / snds)) * dt_glb;
            }
        }
