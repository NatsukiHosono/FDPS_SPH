...
void OutputBinary(PS::ParticleSystem<ThisPtcl> &sph_system, const system_t &sysinfo, std::string &out_dir) {
    //Binary
    char filename[256];
    std::ofstream fout;
...
template<class ThisPtcl>
void
OutputFileWithTimeInterval(PS::ParticleSystem<ThisPtcl> &sph_system, system_t &sysinfo, const PS::F64 output_interval,
                           std::string &out_dir) {
...
template<class ThisPtcl>
void InputFileWithTimeInterval(PS::ParticleSystem<ThisPtcl> &sph_system, system_t &sysinfo) {
    FileHeader header;
    char filename[256];
...
template<class ThisPtcl>
void InputBinary(PS::ParticleSystem<ThisPtcl> &sph_system, system_t &sysinfo) {
    char filename[256];
