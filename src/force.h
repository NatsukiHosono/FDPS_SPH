...
namespace STD{
	class CalcDensity{
		...
		void operator () (const EPI::Dens* const ep_i, const PS::S32 Nip, const EPJ::Dens* const ep_j, const PS::S32 Njp, RESULT::Dens* const dens){
			for(PS::S32 i = 0 ; i < Nip ; ++ i){
				...
				#ifdef FLAG_GI
				dens[i].dens = std::max(500.0, dens[i].dens); // setting the minimum density. See Hull et al 2023 (?)
				#endif
    ...
    void CalcMomentum(PS::ParticleSystem<STD::RealPtcl> &sph_system) {
        ...
        PS::F64 mom2 = momentum * momentum;
        PS::F64 momentum_total = sqrt(mom2);
        ...
        void operator () (const EPI::Grav* const __restrict ep_i, const PS::S32 Nip, const TPtclJ* const __restrict ep_j, const PS::S32 Njp, RESULT::Grav* const grav){
            for(PS::S32 i = 0; i < Nip ; ++ i){
                ...
                for(PS::S32 j = 0; j < Njp ; ++ j){
                    ...
                    const PS::F64 s = sqrt(dr2) / H;
                    const PS::F64 k = kernel.intWs2(dr, ith.smth);
                    PS::F64 m_j = 4.0 * math::pi * math::pow3(H) * jth.mass * k;
                    if (1 - s < 0) {
                        m_j = jth.mass;
                    }
...
#pragma omp parallel for
        for(PS::S32 i = 0 ; i < sph_system.getNumberOfParticleLocal() ; ++ i){
            sph_system[i].snds = sph_system[i].EoS->SoundSpeed(sph_system[i].dens, sph_system[i].eng, 120);
        }
    }
        ...
        void operator () (const EPI::Hydro* const ep_i, const PS::S32 Nip, const EPJ::Hydro* const ep_j, const PS::S32 Njp, RESULT::Hydro* const hydro){
            for(PS::S32 i = 0; i < Nip ; ++ i){
                ...
                for(PS::S32 j = 0; j < Njp ; ++ j){
                    ...
                    const PS::F64 v_sig = ith.snds + jth.snds - 3.0 * w_ij;
                    v_sig_max = std::max(v_sig_max, v_sig);
                    PS::F64 AV = - 0.5 * v_sig * w_ij / (0.5 * (ith.dens_smth + jth.dens_smth)) * 0.5 * (ith.Bal + jth.Bal);
                    if(PARAM::FLAG_R00 == true){
                        AV *= 0.5 * (ith.AVa + jth.AVa);
                    }
#if 1
